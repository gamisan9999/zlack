...
## セキュリティに関わるコードは TDD で書く

以下に該当するコードを新規実装・変更する場合は、**先にテストを書いてから実装する**:

- 入力バリデーション / sanitize（パストラバーサル、インジェクション）
- 認証・認可（トークン検証、Keychain 操作）
- 外部データのパース（JSON、WebSocket メッセージ）
- ファイル操作（パス構築、読み書き）

手順: テスト作成 → テスト失敗確認 → 実装 → テスト通過確認

## コミット前にテストを通す

`zig build test` が通らない状態でコミットしない。
`.githooks/pre-commit` が自動で検証する。
