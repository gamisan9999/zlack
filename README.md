...
- Japanese input support (UTF-8 codepoint-aware cursor)
...
src/
  ...
  platform/
    keychain.zig      # macOS Keychain integration
```
...
```bash
devenv shell
zig test src/slack/types.zig
zig test src/slack/auth.zig
zig test src/store/cache.zig
zig test src/tui/input.zig
zig test src/tui/messages.zig
```
